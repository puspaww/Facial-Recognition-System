...
class Student:
    def __init__(self, root):
        ...
        take_sample_button=Button(button_frame1,command=self.generate_dataset,text="Take Photo Sample",width=31,font=("Comic Sans MS", 11, "bold"), bg= "grey")
    ...
    def generate_dataset(self):
        if self.var_dep.get()=="Select Department" or self.var_name.get()=="" or self.var_id.get()=="":
            ...
            try:  
                ...
                for x in myresult:
                    id+=1
                my_cursor.execute("update student set Department=%s, Year=%s, Semester=%s, Course=%s, Name=%s, DOB=%s,Email=%s,Address=%s,Phone=%s,Gender=%s, Photo_Sample=%s where Student_ID=%s",(self.var_dep.get(),self.var_year.get(),self.var_sem.get(),self.var_course.get(),self.var_name.get(),self.var_dob.get(),self.var_email.get(),self.var_address.get(),self.var_phone.get(),self.var_gender.get(),self.var_radio1.get(),self.var_id.get()==id+1))                                                                                                                                                          
                conn.commit()
                self.fetch_data()
                self.reset_data()
                conn.close()
