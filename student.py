...
class Student:
    def __init__(self, root):
        ...
        Year_combo = ttk.Combobox(Left_frame, textvariable=self.var_year, font=(
            "Comic Sans MS", 11, "bold"), state="read only")
        ...
        # Semester
        Sem_label = Label(Left_frame, text="Semester", font=(
            "Comic Sans MS", 11, "bold"), bg="white")
        Sem_label.grid(row=2, column=0, padx=2, pady=10)

        Sem_combo = ttk.Combobox(Left_frame, textvariable=self.var_sem, font=(
            "Comic Sans MS", 11, "bold"), state="read only")
        ...
        Course_entry = ttk.Entry(Left_frame, textvariable=self.var_course, width=20, font=(
        ...
        StudentID_entry = ttk.Entry(
            Leftbottom_frame, textvariable=self.var_id, width=20, font=("Comic Sans MS", 11, "bold"))
        ...
        StudentName_entry = ttk.Entry(
            Leftbottom_frame, textvariable=self.var_name, width=20, font=("Comic Sans MS", 11, "bold"))
        StudentName_entry.grid(row=1, column=1, padx=2, pady=10, sticky=W)

        # DOB
        DOB_label = Label(Leftbottom_frame, text="DOB:", font=(
            "Comic Sans MS", 11, "bold"), bg="white")
        DOB_label.grid(row=0, column=2, padx=2, pady=10, sticky=W)

        DOB_entry = ttk.Entry(Leftbottom_frame, textvariable=self.var_dob, width=20, font=(
            "Comic Sans MS", 11, "bold"))
        DOB_entry.grid(row=0, column=3, padx=2, pady=10, sticky=W)

        # email
        Email_label = Label(Leftbottom_frame, text="Email:", font=(
            "Comic Sans MS", 11, "bold"), bg="white")
        Email_label.grid(row=1, column=2, padx=2, pady=10, sticky=W)

        Email_entry = ttk.Entry(Leftbottom_frame, textvariable=self.var_email, width=20, font=(
            "Comic Sans MS", 11, "bold"))
        Email_entry.grid(row=1, column=3, padx=2, pady=10, sticky=W)

        # Phone
        Phone_label = Label(Leftbottom_frame, text="Phone:", font=(
            "Comic Sans MS", 11, "bold"), bg="white")
        Phone_label.grid(row=2, column=2, padx=2, pady=10, sticky=W)

        Phone_entry = ttk.Entry(Leftbottom_frame, textvariable=self.var_phone, width=20, font=(
            "Comic Sans MS", 11, "bold"))
        Phone_entry.grid(row=2, column=3, padx=2, pady=10, sticky=W)

        # Address
        Address_label = Label(Leftbottom_frame, text="Address:", font=(
            "Comic Sans MS", 11, "bold"), bg="white")
        Address_label.grid(row=2, column=0, padx=2, pady=10, sticky=W)

        Address_entry = ttk.Entry(Leftbottom_frame, textvariable=self.var_address, width=20, font=(
            "Comic Sans MS", 11, "bold"))
        Address_entry.grid(row=2, column=1, padx=2, pady=10, sticky=W)

        # Gender
        Gender_label = Label(Leftbottom_frame, text="Gender:", font=(
            "Comic Sans MS", 11, "bold"), bg="white")
        Gender_label.grid(row=3, column=0, padx=2, pady=10, sticky=W)

        Gender_entry = ttk.Entry(Leftbottom_frame, textvariable=self.var_gender, width=20, font=(
            "Comic Sans MS", 11, "bold"))
        ...
        radio_button_1 = ttk.Radiobutton(
            Leftbottom_frame, variable=self.var_radio1, text="Take Photo Sample", value="Yes")
        radio_button_1.grid(row=4, column=0)

        self.var_radio2 = StringVar()
        radio_button_2 = ttk.Radiobutton(
            Leftbottom_frame, variable=self.var_radio2, text="Do not take Photo Sample", value="No")
        ...
        save_button = Button(button_frame, text="Save", command=self.add_data, width=15, font=(
            "Comic Sans MS", 11, "bold"), bg="grey")
        save_button.grid(row=0, column=0)

        update_button = Button(button_frame, text="Update", width=16, command=self.update_data, font=(
            "Comic Sans MS", 11, "bold"), bg="grey")
        update_button.grid(row=0, column=1)

        delete_button = Button(button_frame, text="Delete", command=self.delete_data, width=15, font=(
            "Comic Sans MS", 11, "bold"), bg="grey")
        delete_button.grid(row=0, column=2)

        reset_button = Button(button_frame, text="Reset", command=self.reset_data, width=15, font=(
            "Comic Sans MS", 11, "bold"), bg="grey")
        ...
        take_sample_button = Button(button_frame1, command=self.generate_dataset,
                                    text="Take Photo Sample", width=31, font=("Comic Sans MS", 11, "bold"), bg="grey")
        take_sample_button.grid(row=0, column=0)

        Update_sample_button = Button(button_frame1, command=self.Update_dataset, text="Update Photo Sample", width=32, font=(
            "Comic Sans MS", 11, "bold"), bg="grey")
        ...
        Search_frame = LabelFrame(Right_frame, bg="white", relief=RIDGE,
                                  text="Search System", font=("Comic Sans MS", 11, "bold"))
        ...
        search_combo = ttk.Combobox(Search_frame, width=15, font=(
            "Comic Sans MS", 11, "bold"), state="read only")
        ...
        search_entry = ttk.Entry(
            Search_frame, width=15, font=("Comic Sans MS", 11, "bold"))
        search_entry.grid(row=0, column=2, padx=2, pady=10, sticky=W)

        search_button = Button(Search_frame, text="Search", font=(
            "Comic Sans MS", 11, "bold"), bg="grey")
        search_button.grid(row=0, column=3)

        ShowAll_button = Button(Search_frame, text="Show All", font=(
            "Comic Sans MS", 11, "bold"), bg="grey")
        ...
        self.student_table = ttk.Treeview(table_frame, columns=("Department", "Year", "Semester", "Course", "Student_ID", "Name",
                                          "DOB", "Email", "Address", "Phone", "Gender", "Photo_Sample"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_Y.set)
        ...
        if self.var_dep.get() == "Select Department" or self.var_name.get() == "" or self.var_id.get() == "":
            ...
            try:
                Upadate = messagebox.askyesno(
                    "Update", "Do you want to update the student details?", parent=self.root)
                if Upadate > 0:
    ...
    def generate_dataset(self):
        if self.var_dep.get() == "Select Department" or self.var_name.get() == "" or self.var_id.get() == "":
            ...
            try:
                ...
                my_cursor.execute("update student set Department=%s, Year=%s, Semester=%s, Course=%s, Name=%s, DOB=%s,Email=%s,Address=%s,Phone=%s,Gender=%s, Photo_Sample=%s where Student_ID=%s", (self.var_dep.get(), self.var_year.get(
                ), self.var_sem.get(), self.var_course.get(), self.var_name.get(), self.var_dob.get(), self.var_email.get(), self.var_address.get(), self.var_phone.get(), self.var_gender.get(), self.var_radio1.get(), self.var_id.get() == id+1))
                conn.commit()
                self.fetch_data()
                self.reset_data()
                conn.close()
    ...
    def Update_dataset(self):
        if self.var_dep.get() == "Select Department" or self.var_name.get() == "" or self.var_id.get() == "":
            ...
            try:
                ...
                messagebox.showinfo(
                    "Result", "Photo Samples Updated!", parent=self.root)
            except Exception as es:
                messagebox.showerror(
                    "Error", f"Due to {str(es)}", parent=self.root)
